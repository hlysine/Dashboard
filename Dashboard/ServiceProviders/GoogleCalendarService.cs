...
namespace Dashboard.ServiceProviders
{
    public class GoogleCalendarService : AuthCodeServiceProvider
    {
        ...
        public async Task<Dictionary<CalendarListEntry, List<Event>>> GetAllEvents(DateTime timeMin = default, int maxResults = 100)
        {
            var calendars = await GetAllCalendars();

            var ret = new Dictionary<CalendarListEntry, List<Event>>();
            foreach (var calendar in calendars.Items)
            {
                var events = await GetEvents(calendar.Id, timeMin, maxResults);
                ret.Add(calendar, events.Items.ToList());
            }
